var jsPsychExtensionMouseTracking = (function (t) {
  "use strict";
  var l = {
    ...
    h = (c, a, e) =>
      new Promise((r, s) => {
        var i = (n) => {
          ...
          o = (n) => (n.done ? r(n.value) : Promise.resolve(n.value).then(i, d));
        o((e = e.apply(c, a)).next());
      });
  class u {
    constructor(a) {
      (this.jsPsych = a),
        (this.initialize = (e) =>
          h(this, [e], function* ({ minimum_sample_time: r = 0 }) {
            (this.domObserver = new MutationObserver(this.mutationObserverCallback)),
              (this.minimumSampleTime = r);
          })),
        (this.on_start = (e) => {
          (e = e || {}),
        ...
        (this.mouseMoveEventHandler = ({ clientX: e, clientY: r }) => {
          const s = performance.now(),
            i = Math.round(s - this.currentTrialStartTime);
          (this.lastSampleTime === null || s - this.lastSampleTime >= this.minimumSampleTime) &&
            ((this.lastSampleTime = s),
            this.currentTrialData.push({ x: e, y: r, t: i, event: "mousemove" }));
        }),
        (this.mouseUpEventHandler = ({ clientX: e, clientY: r }) => {
          const s = performance.now(),
            i = Math.round(s - this.currentTrialStartTime);
          this.currentTrialData.push({ x: e, y: r, t: i, event: "mouseup" });
        }),
        (this.mouseDownEventHandler = ({ clientX: e, clientY: r }) => {
          const s = performance.now(),
            i = Math.round(s - this.currentTrialStartTime);
          this.currentTrialData.push({ x: e, y: r, t: i, event: "mousedown" });
        }),
        (this.mutationObserverCallback = (e, r) => {
          for (const s of this.currentTrialSelectors)
            if (!this.currentTrialTargets.has(s)) {
              const i = this.jsPsych.getDisplayElement().querySelector(s);
              i && this.currentTrialTargets.set(s, i.getBoundingClientRect());
            }
        });
    }
  }
      ...
      data: {
        mouse_tracking_data: {
          ...
          nested: {
            ...
            t: { type: t.ParameterType.INT },
            event: { type: t.ParameterType.STRING },
          },
        },
        mouse_tracking_targets: {
          ...
          nested: {
            x: { type: t.ParameterType.INT },
            y: { type: t.ParameterType.INT },
            width: { type: t.ParameterType.INT },
            height: { type: t.ParameterType.INT },
            top: { type: t.ParameterType.INT },
            bottom: { type: t.ParameterType.INT },
            left: { type: t.ParameterType.INT },
            right: { type: t.ParameterType.INT },
          },
        },
      },
    }),
