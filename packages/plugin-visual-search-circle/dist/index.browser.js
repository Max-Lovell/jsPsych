  ...
  class VisualSearchCirclePlugin {
    ...
    trial(display_element, trial) {
      ...
      const show_search_array = () => {
        ...
        const after_response = (info2) => {
          var correct = false;
          if (
            (this.jsPsych.pluginAPI.compareKeys(info2.key, trial.target_present_key) &&
              trial.target_present) ||
            (this.jsPsych.pluginAPI.compareKeys(info2.key, trial.target_absent_key) &&
              !trial.target_present)
          ) {
            correct = true;
          }
    ...
    generateDisplayLocs(n_locs, trial) {
      ...
      for (var i = 0; i < n_locs; i++) {
        display_locs.push([
          Math.floor(
            paper_size / 2 + this.cosd(random_offset + i * (360 / n_locs)) * radi - hstimw
          ),
          Math.floor(
            paper_size / 2 - this.sind(random_offset + i * (360 / n_locs)) * radi - hstimh
          ),
        ]);
      }
      return display_locs;
